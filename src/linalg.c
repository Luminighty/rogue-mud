...
inline bool rect2i_contains(Rect2i rect, int x, int y) {
	return x >= rect.x &&
		y >= rect.y &&
		x <= rect.x + rect.w &&
		y <= rect.y + rect.h;
}
inline Vec2i rect2i_center(Rect2i rect) {
	return ((Vec2i){ .x = rect.x + rect.w / 2, .y = rect.y + rect.h / 2 });
}
...
inline bool rect2_contains(Rect2 rect, float x, float y) {
	return x >= rect.x &&
		y >= rect.y &&
		x <= rect.x + rect.w &&
		y <= rect.y + rect.h;
}
inline Vec2 rect2_center(Rect2 rect) {
	return ((Vec2){ .x = rect.x + rect.w / 2.0f, .y = rect.y + rect.h / 2.0f });
}
